...
public class ParameterBindingTests {
    ...
    * */
    ...
    void 이름_기준_파라미터_바인딩_메뉴_조회_테스트(){
        ...
        String menuNameParameter = "해산물오일파스타";
    ...
    void 위치_기준_파라미터_바인딩_메뉴_목록_조회_테스트(){
        ...
        String jpql = "SELECT m FROM menu_section02 m WHERE m.menuName = ?1";  // 1부터 시작해서 증가시킨다!
...
//
//    String menuNameParameter = scr;
//
//    String jpql = "SELECT m FROM menu_section02 m WHERE m.menuName = :scr";
//    List<Menu> menuList = entityManager.createQuery(jpql, Menu.class).setParameter(scr, menuNameParameter).getResultList();
//
//    Assertions.assertNotNull(menuList);
//    for(Menu menu:menuList){
//        System.out.println(menu);
//    }
    ...
    void 테스트(){
        ...
        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class).setParameter("menuName", "%" + name + "%")
                .getResultList();
        Assertions.assertNotNull(menuList);
        for(Menu menu:menuList){
            System.out.println(menu);
