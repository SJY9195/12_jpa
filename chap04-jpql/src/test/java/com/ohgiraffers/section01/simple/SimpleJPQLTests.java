...
public class SimpleJPQLTests {
    ...
    * */
    ...
    void Query_이용한_단일메뉴_조회_테스트(){
        ...
        String jpql = "SELECT m.menuName FROM menu_section01 AS m WHERE m.menuCode = 1";
        Query query = entityManager.createQuery(jpql);

        Object resultMenuName = query.getSingleResult();
        Assertions.assertTrue(resultMenuName instanceof String);
        Assertions.assertEquals("해산물오일파스타", resultMenuName);
    }

    @Test
    void Query를_이용한_다중행_조회_테스트(){
        String jpql ="SELECT m.menuName FROM menu_section01 AS m";  // 회사에선 별칭을 꼭 넣는다!
        Query query = entityManager.createQuery(jpql);
    ...
    void TypedQuery를_이용한_단일행_조회_테스트(){
        ...
        String jpql = "SELECT m FROM menu_section01 AS m WHERE m.menuCode = 1"; // *은 안되고 별칭을 넣어줘야 한다!!
        TypedQuery<Menu> query = entityManager.createQuery(jpql, Menu.class);

        Menu foundMenu = query.getSingleResult();

        Assertions.assertEquals(1, foundMenu.getMenuCode());
    ...
    void distinct_활용한_중복제거_여러행_조회_테스트(){
        ...
        for (int categoryCode : categoryList) {
    ...
    void 카테고리_코드가_6번_10번인_메뉴_조회(){
        ...
        String jpql = "SELECT m FROM menu_section01 AS m WHERE m.categoryCode IN (6, 10)"; //IN 은 안에 있는 것만 보여주는것! but Like는 포함된 것을 보여주는것!
        TypedQuery<Menu> query = entityManager.createQuery(jpql, Menu.class);
    ...
    void 메뉴_이름에_마늘이_포함된_메뉴(){
        ...
        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class).getResultList();  //메소드체이닝에 의해 이렇게 축약이 가능하다!
...
/*
* Mybatis 는 복잡한 SQL 쿼리와 데이터베이스 성능 최적화 측면에서,
...
*
* Mybatis 는 복잡한 조인, 서브쿼리, 다양한 함수 등을 처리 할 때 더 유연하고,
* 쿼리의 복잡성을 완전히 개발자가 컨트롤할 수 있다.
* SQL 을 직접 작성하여 사용하므로, 객체 변환 없이 순수 SQL 을 실행 할 수 있다.
* jpa 는 orm 을 통해 객체로 변환하는 과정을 거쳐야 하고, 복잡해질수록 성능적 문제와
* 추가적인 시간이 소요된다.
*
* */
