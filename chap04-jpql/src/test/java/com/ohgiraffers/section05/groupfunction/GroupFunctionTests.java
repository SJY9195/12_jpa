...
public class GroupFunctionTests {
    ...
    * */
    ...
    void count_제외_조회결과_없을시_테스트(){
        ...
        Long sumOfPrice = entityManager.createQuery(jpql, Long.class).setParameter("categoryCode", categoryCode).getSingleResult();
        // 여기서는 조건이 딱히 없고, 단순히 SUM을 해주는거니 검증이 필요없으므로 Assertions을 해줄 필요가 없다!
    ...
    void groupby절과_having절을_사용한_조회_테스트(){
        ...
        List<Object[]> sumPriceOfCategoryList = entityManager.createQuery(jpql, Object[].class).setParameter("minPrice", minPrice).getResultList();  //cf)타입쿼리를 굳이 안넣어도된다
