...
public class ProjectionTests {
    ...
    * - new 명령어를 사용한 클래스의 객체는 엔티티가 아니므로 영속성 컨텍스트에서 관리되지 않는다.  // 쿼리문에 경로를 넣어주는것!(거의 쓸일이 없다.)
    *
    * */
    ...
    void 단일_엔티티_프로젝션_테스트(){
        ...
        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class).getResultList();  // 엔티티가 조회되면 결과값을 영속성 컨텍스트에 등록하게 되어 관리하게 된다!

        Assertions.assertNotNull(menuList);
        EntityTransaction entityTransaction = entityManager.getTransaction();
        entityTransaction.begin();
        menuList.get(1).setMenuName("메뉴이름 변경 test");   //그래서 영속성이기 때문에 메뉴이름이 변경 가능한것!
    ...
    void 임베디드_프로젝션_테스트(){
        ...
        * 중복을 방지하려면 테이블에서 유니크 제약 조건을 추가해야 한다.    //@Embeddable은 단순히 그룹화 역할을 한다 (여러필드를 하나의 객체로 묶어 표현하는 방식)
        *
    ...
    * */
    ...
    @Test
    void 스칼라_테스트_조회컬럼_2개(){

        String jpql = "SELECT c.categoryCode, c.categoryName FROM category_section03 c";
        List<Object[]> categoryList = entityManager.createQuery(jpql).getResultList();  // jpql, ~.class 에서 ~.class는 안쓰는게 그냥 쿼리이고, 자료형 있는게 타입쿼리이다! // 이 코드는 그냥 쿼리이다!
...
//   다른답 List<Object> categoryList =entityManager.createQuery(jpql).getResultList();
