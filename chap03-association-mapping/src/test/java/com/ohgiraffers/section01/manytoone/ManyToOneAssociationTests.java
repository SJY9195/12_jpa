...
public class ManyToOneAssociationTests {
    ...
    * */
    ...
    void 다대일_연관관계_객체_삽입_테스트(){
        ...
        menuAndCategory.setCategory(category);   // 카테고리에 위의 것이 존재하지 않기 때문에 오류가 나서 등록이 안된다! 즉, 없는걸 참조하고 등록하려해서 안된다!
        menuAndCategory.setOrderableStatus("Y");

        EntityTransaction entityTransaction = entityManager.getTransaction();
        entityTransaction.begin();
        entityManager.persist(menuAndCategory);  //menuAndCategory는 영속화 되어있으니 참조할 것이 영속화 안되있으므로 참조할 걸 못찾아서 커밋이 안된다!
        entityTransaction.commit();                     //하지만 cascade 하면 참주 주는애 (부모)와 까지 자동으로 영속화가 된다!

        MenuAndCategory foundMenuAndCategory =
                entityManager.find(MenuAndCategory.class, 99999);
    ...
    void Merge_Insert_테스트(){
        ...
        menuAndCategory.setOrderableStatus("Y");
        Category category = new Category();
        category.setCategoryName("merge카테고리");    //merge 기존것이 있으면 기존것을 바꾸고 병합, 없으면 새롭게 넣어준다!
    ...
    void detach_테스트(){
        ...
        * */
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class, 99999);
        menuAndCategory.setMenuName("변경함"); //준영속화 이므로 안바뀐다.  //menuAndCategory는 JPA가 영속성 컨텍스트에서 해당 객체를 찾아 반환하는 방식이고, 위 find 된것을 DB에서 존재하는 엔티티(해당 컬럼의 행)을 가져온다!
        Category category = menuAndCategory.getCategory();
        category.setCategoryName("진짜 안바뀌나? ");   // category는 준영속화된 menuAndCategory메소드를 써도 준영속화 설정을 안해주면 영속상태이다! 준영속화는 해당 컬럼 DTO에 어노테이션으로 설정가능하다!
        menuAndCategory.setCategory(category);
