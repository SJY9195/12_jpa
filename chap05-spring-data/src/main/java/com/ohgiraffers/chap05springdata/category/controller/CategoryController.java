...
@RestController
@RequestMapping("/category") // /* 를하면 정확한 경로를 못찾아 충돌나서 오류가 날 수 있으므로 RestAPI 설계 할 때는 *생략하는게 좋다!
public class CategoryController {
    ...
    * 에를 들어 특정 조건에서 다른 HTTP 상태 코드를 반환하고 싶거나, 헤더를 추가하고 싶다면, 이 방식은 적합하지 않다.
    * */

    @GetMapping("/select/{categoryCode}")
    public ResponseEntity<Object> selectCategoryByCode(@PathVariable Integer categoryCode){
    ...
    public ResponseEntity<Object> updateCategory(@RequestBody Category category){
        ...
        // 유효성 검사: categoryCode와 categoryName 필수 확인
        if (Objects.isNull(categoryName)){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("카테고리 코드와 이름은 필수입니다.");
        }
    ...
    // 카테고리 삭제
    @PostMapping("/delete")
    public ResponseEntity deleteCateogry(@RequestBody HashMap<String, Integer> requestData){
