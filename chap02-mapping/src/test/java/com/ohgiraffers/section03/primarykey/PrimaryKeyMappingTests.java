...
public class PrimaryKeyMappingTests {
    ...
    * - GenerationType.IDENTITY : 기본 키 생성을 데이터베이스에 위임 (Mysql 의 AutoIncrement) // 보통 이걸로 쓴다
    * - GenerationType.SEQUENCE : 데이터베이스 시퀀스 객체 사용 (ORACLE 의 SEQUENCE)
    * - Generation.TABLE : 키 생성 테이블 사용
    ...
    * */
    ...
    public void 식별자_매핑_테스트(){
        ...
        entityManager.persist(member); // persist는 한번에 하나만 넣을 수 있다! 반복문이나 따로 또 써줘야 한다!
        entityManager.persist(member2);
        entityTransaction.commit();

        String jpql = "SELECT A.memberNo FROM member_section03 A"; // 데이터베이스에 영향을 받지 않는다! (엔티티를 기준이므로!)
